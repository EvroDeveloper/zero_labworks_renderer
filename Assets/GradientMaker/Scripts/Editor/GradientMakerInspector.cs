...
class GradientMakerInspector : Editor {
	...
	public override void OnInspectorGUI(){
        ...
        gradEdit._fixedAspect = EditorGUILayout.Toggle("FixedAspect", gradEdit._fixedAspect);


        //EditorGUILayout.PrefixLabel("Gradient Size");
        GUILayout.Label("Gradient Size", EditorStyles.boldLabel);
        EditorGUILayout.BeginHorizontal();
			...
			if(!gradEdit._fixedAspect){
                ...
                gradEdit._gradientSizeXY = EditorGUILayout.Vector2IntField("", new Vector2Int(  Mathf.Clamp(gradEdit._gradientSizeXY.x, 8 , 8192) , Mathf.Clamp(gradEdit._gradientSizeXY.y, 8 , 8192)  ) );

			} else {

                gradEdit._gradientSizeXY.x = EditorGUILayout.IntField("", Mathf.Clamp(gradEdit._gradientSizeXY.x, 8 , 8192) );
                gradEdit._gradientSizeXY.y = gradEdit._gradientSizeXY.x;
...
//            string gradSizeY = EditorGUILayout.Vector2IntField("Gradient Height", gradEdit._gradientSizeY.ToString());
...
//        }
		...
		guioptions[0] = GUILayout.Height(Mathf.Clamp(gradEdit._gradientSizeXY.x,gradEdit._gradientSizeXY.x, 256));
        guioptions[1] = GUILayout.Width(Mathf.Clamp(gradEdit._gradientSizeXY.x,gradEdit._gradientSizeXY.x, 256));
		...
		Texture2D checker = new Texture2D(gradEdit._gradientSizeXY.x, gradEdit._gradientSizeXY.y);
